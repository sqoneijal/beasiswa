...
const Forms = ({ setPageTypeButton }) => {
   ...
   const submit = (e) => {
      ...
      setIsSubmit(true);
      const fetch = h.post(`/referensi/lampiranupload/submit`, formData);
      fetch.then((res) => {
   ...
   return (
      <Form onSubmit={isSubmit ? null : submit}>
         {h.form_text(
            ...
            `nama`,
            2,
            { onChange: (e) => setInput((prev) => ({ ...prev, nama: e.target.value })), value: h.parse(`nama`, input) },
            errors
         )}
         {h.form_textarea(
            `Keterangan`,
            `keterangan`,
            2,
            { onChange: (e) => setInput((prev) => ({ ...prev, keterangan: e.target.value })), value: h.parse(`keterangan`, input) },
            errors
         )}
